...
# Validation harness for retention Length x Content LMM (R).
#
# Purpose
# - Verify `analyze_retention_format_content_lmm.R` implements:
...
#   - Complete-case rule (subject must have all 4 retention values)
#   - Retention zero handling as valid data (not missing)
...
generate_retention <- function(n_subjects, b0, bL, bC, bI, b_age = 0.10, noise_sd = 0.0, seed = 123, age_years = make_age_years(n_subjects)) {
  set.seed(seed)
  sid <- sprintf("%04d", seq_len(n_subjects))
  subj_re <- seq(-0.35, 0.35, length.out = n_subjects)
  ...
  tibble::tibble(
    subject_id = sid,
    age = age_years,
    diff_short_form_education = make_cell(-0.5, +0.5),
    diff_short_form_entertainment = make_cell(-0.5, -0.5),
    diff_long_form_education = make_cell(+0.5, +0.5),
    diff_long_form_entertainment = make_cell(+0.5, -0.5)
  )
}

compute_effects_from_cell_means <- function(df) {
  m_sfe <- mean(df$diff_short_form_education)
  m_sfeent <- mean(df$diff_short_form_entertainment)
  m_lfe <- mean(df$diff_long_form_education)
  m_lfent <- mean(df$diff_long_form_entertainment)
...
run_analysis <- function(input_csv, out_main, out_posthoc, alpha = 0.05, min_subjects = 6, exclude_json = NULL) {
  ...
  main <- function() {
  tmp <- file.path(tempdir(), "retention_pipeline_validation")
  ...
  assert_true(abs(eff_det$length - 0.55) < 1e-12, "analytic length effect mismatch")
  assert_true(abs(eff_det$content - (-0.35)) < 1e-12, "analytic content effect mismatch")
  assert_true(abs(eff_det$interaction - 1.20) < 1e-12, "analytic interaction effect mismatch")

  # 2) End-to-end run with stochastic noise and known generating parameters.
  n_subjects <- 24
  truth <- list(length = 0.50, content = -0.20, interaction = 1.10)
  age_years <- make_age_years(n_subjects)
  df <- generate_retention(
  ...
  input_csv <- file.path(tmp, "retention.csv")
  ...
  ref_long <- df %>%
    select(subject_id, age, diff_short_form_education, diff_short_form_entertainment, diff_long_form_education, diff_long_form_entertainment) %>%
    pivot_longer(
      cols = c(diff_short_form_education, diff_short_form_entertainment, diff_long_form_education, diff_long_form_entertainment),
      names_to = "diff_col",
      values_to = "retention_diff"
    ) %>%
    mutate(
      condition = case_when(
  ...
  ref_model <- lmerTest::lmer(retention_diff ~ length_c * content_c + age + (1 | subject_id), data = ref_long, REML = TRUE)
  ...
  expected_diff <- mean(df$diff_short_form_education - df$diff_short_form_entertainment)
  ...
  # 7) Zero as valid retention value: should not trigger complete-case drop.
  df_zero <- df
  df_zero$diff_long_form_entertainment[[1]] <- 0
  input_zero <- file.path(tmp, "retention_zero_valid.csv")
  ...
  assert_true(run_zero$status == 0, "analysis failed with valid zero retention value")
  main_zero <- read_csv(out_main_zero, show_col_types = FALSE)
  assert_true(all(main_zero$n_subjects == n_subjects), "valid zero retention value should not reduce subject count")

  # 8) Complete-case drop on NA in one condition.
  df_na <- df
  df_na$diff_long_form_entertainment[[1]] <- NA_real_
  input_na <- file.path(tmp, "retention_na.csv")
  ...
  input_dup <- file.path(tmp, "retention_dup.csv")
  ...
  df_missing <- df %>% select(-diff_long_form_education)
  input_missing <- file.path(tmp, "retention_missing_col.csv")
  ...
  input_missing_age <- file.path(tmp, "retention_missing_age.csv")
  ...
  input_age_na <- file.path(tmp, "retention_age_na.csv")
  ...
  input_age_bad <- file.path(tmp, "retention_age_bad.csv")
  ...
  # 12) Fail hard on non-numeric retention entry.
  df_bad <- df
  df_bad$diff_short_form_education <- as.character(df_bad$diff_short_form_education)
  df_bad$diff_short_form_education[[2]] <- "not_numeric"
  input_bad <- file.path(tmp, "retention_non_numeric.csv")
  ...
  assert_true(run_bad$status != 0, "expected failure on non-numeric retention value")

  writeLines(paste0("[OK] Retention pipeline validation passed. Outputs in: ", tmp))
}
